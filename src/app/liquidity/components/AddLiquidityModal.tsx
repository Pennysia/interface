...
import { MARKET_ABI, ERC20_ABI, LIQUIDITY_ABI } from "../../../lib/abis";
...
export default function AddLiquidityModal({
  ...
  const poolPrice =
    poolReserves && selectedTokenA && selectedTokenB
      ? Number(
          ethers.formatUnits(poolReserves.reserve1, selectedTokenB.decimals)
        ) /
        Number(
          ethers.formatUnits(poolReserves.reserve0, selectedTokenA.decimals)
        )
      : 0;
  ...
  // Smart number formatting with compact notation for very small numbers
    ...
    try {
      ...
      if (finalParts.length > 1 && finalParts[1].length > 18) {
        return (
          finalParts[0] +
          "." +
          finalParts[1].substring(0, 18).replace(/0+$/, "")
        );
      }
  ...
  // Compact number formatting for display (e.g., 0.0000000001234565 -> 0.0{8}12345)
  const formatCompactNumber = (value: string | number): string => {
    try {
      ...
      // For very small numbers, use compact notation
      const str = num.toFixed(20);
      const parts = str.split(".");

      if (parts.length !== 2) return formatSafeNumber(value);
      ...
      // If we have leading zeros, use compact notation
      if (leadingZeros > 0) {
        // Get all available significant digits after leading zeros
        const significantDigits = decimals.substring(leadingZeros);
        // Remove trailing zeros from significant digits
        const cleanSignificant = significantDigits.replace(/0+$/, "");

        if (cleanSignificant.length > 0) {
          // Always pad to exactly 5 digits
          const paddedSignificant = cleanSignificant
            .padEnd(5, "0")
            .substring(0, 5);
          return `0.0{${leadingZeros}}${paddedSignificant}`;
        }
      }

      // For numbers with no leading zeros (like 0.34), format normally with 5 decimal places
      if (leadingZeros === 0) {
        const significantDigits = decimals;
        const cleanSignificant = significantDigits.replace(/0+$/, "");
      ...
      console.error("Error formatting compact number:", error, "Input:", value);
      return formatSafeNumber(value);
    }
  };
  ...
  const calculateDecimalAwarePrice = useCallback(() => {
    ...
    try {
      ...
      // Calculate exchange rates accounting for decimal differences
      // Price = amountB / amountA, but we need to normalize for decimals
      // If tokenA has 18 decimals and tokenB has 6 decimals:
      // 1 tokenA unit = 10^18 smallest units
      // 1 tokenB unit = 10^6 smallest units
      // So price needs to be adjusted by the decimal difference

      const decimalDifference =
        selectedTokenB.decimals - selectedTokenA.decimals;
      const decimalAdjustment = Math.pow(10, decimalDifference);

      // Raw price without decimal adjustment
      const rawTokenAToB = amountBNum / amountANum;
      const rawTokenBToA = amountANum / amountBNum;

      // Apply decimal adjustment
      const tokenAToB = rawTokenAToB * decimalAdjustment;
      const tokenBToA = rawTokenBToA / decimalAdjustment;
  ...
  const checkPoolExistence = useCallback(async () => {
    ...
    try {
      ...
      if (!marketAddress) {
        console.error(
          `Market address not found for chain ID ${CURRENT_CHAIN_ID}`
        );
      ...
      try {
        // First get the pair ID for the token pair
        const pairId = await marketContract.getPairId(
          token0.address,
          token1.address
        );
        console.log(
        ...
        // Then get reserves using the pair ID
        const reserves = await marketContract.getReserves(pairId);
        ...
        console.log(
          "📝 Pool does not exist yet (getReserves failed):",
      ...
      try {
        ...
        if (selectedTokenA) {
          ...
          if (selectedTokenA.address === ethers.ZeroAddress) {
            ...
            // ERC20 token - use contract.balanceOf()
          ...
          const formattedBalanceA = ethers.formatUnits(
            balanceA,
        ...
        if (selectedTokenB) {
          ...
          if (selectedTokenB.address === ethers.ZeroAddress) {
            // Native token - use provider.getBalance()
          ...
          const formattedBalanceB = ethers.formatUnits(
            balanceB,
  ...
  const canProceedToStep3 =
    ...
    Number(amountB) > 0;
  const canComplete =
    canProceedToStep3 && bullishPercentage + bearishPercentage === 100;
              ...
              {selectedTokenA ? (
                <>
                  <div className="w-6 h-6 rounded-full flex items-center justify-center overflow-hidden">
                    {selectedTokenA.address === ethers.ZeroAddress ? (
                      <Image
                        src="/sonic-logo.avif"
                        alt="Sonic"
                        width={24}
          ...
          >
            <div className="flex items-center space-x-3">
              {selectedTokenB ? (
                <>
                  <div className="w-6 h-6 rounded-full flex items-center justify-center overflow-hidden">
                    {selectedTokenB.address === ethers.ZeroAddress ? (
                      <Image
                        src="/sonic-logo.avif"
                        alt="Sonic"
                        width={24}
  ...
  const renderStep2 = () => (
    <div className="space-y-6">
      <div className="text-center">
        ...
        <p className="text-gray-400">
          {poolReserves
            ? "Enter amounts (ratio will be enforced)"
                  ...
                  onClick={() => {
                    if (maxAmountA) {
                      ...
                      if (
                        ...
                        if (isTokenAFirst) {
                          expectedB =
                            (Number(maxAmountA) * reserve1Formatted) /
                            reserve0Formatted;
                        } else {
                          expectedB =
                            (Number(maxAmountA) * reserve0Formatted) /
                            reserve1Formatted;
                        }
                        ...
                        if (maxAmountB && expectedB > Number(maxAmountB)) {
                          ...
                          if (isTokenAFirst) {
                            recalculatedA =
                              (cappedB * reserve0Formatted) / reserve1Formatted;
                          } else {
                            recalculatedA =
                              (cappedB * reserve1Formatted) / reserve0Formatted;
                          }
                          setAmountA(
                            formatSafeNumber(recalculatedA.toString())
                          );
                          toast.success(
                            "Both amounts set to max within balance limits"
                          );
                        } else {
                          setAmountB(formatSafeNumber(expectedB.toString()));
                          toast.success(
                            `Set Token A to max: ${formatSafeNumber(
                              maxAmountA
                            )}`
                          );
                        }
                        ...
                        setAmountB("");
                        toast.success(
                          `Set Token A to max: ${formatSafeNumber(maxAmountA)}`
                        );
                      }
                    }
                  }}
                >
                  Max: {formatCompactNumber(maxAmountA)}
                </button>
              </span>
            )}
          </label>
            ...
            <input
              ...
              onChange={(e) => {
                ...
                if (maxAmountA && numValue > Number(maxAmountA)) {
                  ...
                  toast.success(
                    `Amount capped at max balance: ${formatSafeNumber(
                      maxAmountA
                    )}`
                  );
                }
                ...
                if (poolReserves && cappedValue && Number(cappedValue) > 0) {
                  ...
                  const reserve0Formatted = Number(
                    ethers.formatUnits(poolReserves.reserve0, token0.decimals)
                  );
                  const reserve1Formatted = Number(
                    ethers.formatUnits(poolReserves.reserve1, token1.decimals)
                  );

                  let expectedB: number;
                  if (isTokenAFirst) {
                    expectedB =
                      (Number(cappedValue) * reserve1Formatted) /
                      reserve0Formatted;
                  } else {
                    expectedB =
                      (Number(cappedValue) * reserve0Formatted) /
                      reserve1Formatted;
                  }
                  ...
                  if (maxAmountB && expectedB > Number(maxAmountB)) {
                    ...
                    toast.success(
                      "Both amounts adjusted to stay within balance limits"
                    );
                  } else {
                    setAmountB(formatSafeNumber(expectedB.toString()));
                  }
                  ...
                  onClick={() => {
                    if (maxAmountB) {
                      ...
                      if (
                        ...
                        if (isTokenAFirst) {
                          expectedA =
                            (Number(maxAmountB) * reserve0Formatted) /
                            reserve1Formatted;
                        } else {
                          expectedA =
                            (Number(maxAmountB) * reserve1Formatted) /
                            reserve0Formatted;
                        }
                        ...
                        if (maxAmountA && expectedA > Number(maxAmountA)) {
                          ...
                          if (isTokenAFirst) {
                            recalculatedB =
                              (cappedA * reserve1Formatted) / reserve0Formatted;
                          } else {
                            recalculatedB =
                              (cappedA * reserve0Formatted) / reserve1Formatted;
                          }
                          setAmountB(
                            formatSafeNumber(recalculatedB.toString())
                          );
                          toast.success(
                            "Both amounts set to max within balance limits"
                          );
                          ...
                          toast.success(
                            `Set Token B to max: ${formatSafeNumber(
                              maxAmountB
                            )}`
                          );
                        }
                        ...
                        setAmountA("");
                        toast.success(
                          `Set Token B to max: ${formatSafeNumber(maxAmountB)}`
                        );
                      }
                    }
                  }}
                >
                  Max: {formatCompactNumber(maxAmountB)}
                </button>
              </span>
            )}
          </label>
        ...
        amountB && (
          <div className="bg-gray-200/30 dark:bg-gray-800/30 rounded-lg p-4">
            ...
            <div className="text-green-400 text-sm">
              Current ratio maintained: 1 {selectedTokenA.symbol} ={" "}
              {poolPrice.toLocaleString()} {selectedTokenB.symbol}
            </div>
          </div>
        )}
        ...
        calculatedPrice && (
          <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4">
            <div className="text-sm text-blue-300 mb-2">
              Initial Pool Price (Decimal-Adjusted)
            </div>
            <div className="space-y-2">
              <div className="text-blue-400 text-sm">
                1 {selectedTokenA.symbol} ={" "}
                {calculatedPrice.tokenAToB.toLocaleString(undefined, {
                  maximumFractionDigits: 8,
                })}{" "}
                {selectedTokenB.symbol}
              </div>
              <div className="text-blue-400 text-sm">
                1 {selectedTokenB.symbol} ={" "}
                {calculatedPrice.tokenBToA.toLocaleString(undefined, {
                  maximumFractionDigits: 8,
                })}{" "}
                {selectedTokenA.symbol}
              </div>
            ...
            <div className="text-xs text-blue-300/70 mt-2">
              This exchange rate accounts for token decimals (
              {selectedTokenA.symbol}: {selectedTokenA.decimals},{" "}
              {selectedTokenB.symbol}: {selectedTokenB.decimals})
            </div>
          </div>
        )}
    </div>
    ...
    return (
      <div className="space-y-6">
        <div className="text-center">
          ...
          <p className="text-gray-400">
            Choose your directional bias for this liquidity position
          </p>
        </div>
        ...
        <div className="space-y-4">
          <div className="flex justify-between text-sm text-gray-400 mb-2">
            <span className="text-gray-400">
              Bearish on {selectedTokenA?.symbol}
            </span>
            <span className="text-gray-400">
              Bullish on {selectedTokenA?.symbol}
            </span>
          </div>
          ...
          <div className="text-center text-sm text-gray-300">
            Current: {sliderValue.toFixed(0)}% {selectedTokenA?.symbol} Bullish
            / {(100 - sliderValue).toFixed(0)}% {selectedTokenB?.symbol} Bullish
          </div>
        </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-2">
              <div className="text-xs text-gray-400">
                {selectedTokenA?.symbol} Distribution
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-green-400">Long:</span>
                <span className="text-green-400 font-mono">
                  {formatCompactNumber(amountALong)}
                </span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-red-400">Short:</span>
                <span className="text-red-400 font-mono">
                  {formatCompactNumber(amountAShort)}
                </span>
              </div>
            </div>
            <div className="space-y-2">
              <div className="text-xs text-gray-400">
                {selectedTokenB?.symbol} Distribution
              </div>
              <div className="flex justify-between text-sm">
        ...
        <div className="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg p-4">
          <div className="text-yellow-800 dark:text-yellow-400 text-sm">
            <strong>Position Explanation:</strong> Moving the slider left favors{" "}
            {selectedTokenA?.symbol} bullish positions, while moving right
            favors {selectedTokenB?.symbol} bullish positions. Your liquidity
            will be allocated proportionally across long and short positions for
            both tokens.
          </div>
        </div>
      </div>
                    ...
                    <div className="bg-gray-100 dark:bg-gray-700/50 rounded-lg p-4 space-y-3">
                      <div className="text-center mb-3">
                        ...
                        <p className="text-sm text-gray-600 dark:text-gray-400">
                          {transactionResult.token0Symbol}/
                          {transactionResult.token1Symbol} Pool
                        </p>
                      </div>
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div className="text-center">
                          <div className="text-sm text-gray-600 dark:text-gray-400 mb-1">
                            {transactionResult.token0Symbol} Added
                          </div>
                          <div className="space-y-1">
                            <div className="flex justify-between text-xs">
                              ...
                              <span className="font-mono">
                                {formatCompactNumber(
                                  transactionResult.amount0Long || "0"
                                )}
                              </span>
                            </div>
                            <div className="flex justify-between text-xs">
                              ...
                              <span className="font-mono">
                                {formatCompactNumber(
                                  transactionResult.amount0Short || "0"
                                )}
                              </span>
                            </div>
                          </div>
                        ...
                        <div className="text-center">
                          <div className="text-sm text-gray-600 dark:text-gray-400 mb-1">
                            {transactionResult.token1Symbol} Added
                          </div>
                          <div className="space-y-1">
                            <div className="flex justify-between text-xs">
                              <span className="text-green-600 dark:text-green-400">
                              ...
                              <span className="font-mono">
                                {formatCompactNumber(
                                  transactionResult.amount1Long || "0"
                                )}
                              </span>
                            </div>
                            <div className="flex justify-between text-xs">
                              ...
                              <span className="font-mono">
                                {formatCompactNumber(
                                  transactionResult.amount1Short || "0"
                                )}
                              </span>
                            </div>
                          </div>
                  ...
                  <>
                    ...
                    <div className="bg-red-500/10 border border-red-500/20 rounded-lg p-4">
                      <div className="flex items-center justify-between mb-2">
                        ...
                        {transactionResult.error &&
                          transactionResult.error.length > 100 && (
                            <button
                              onClick={() =>
                                setExpandErrorMessage(!expandErrorMessage)
                              }
                    ...
                    <div className="bg-gray-100 dark:bg-gray-700/50 rounded-lg p-4 space-y-3">
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        ...
                        <div className="text-center">
                          <div className="text-sm text-gray-600 dark:text-gray-400 mb-1">
                            {transactionResult.token1Symbol}
                          </div>
                          <div className="space-y-1">
                            <div className="flex justify-between text-xs">
                              <span className="text-green-600 dark:text-green-400">
