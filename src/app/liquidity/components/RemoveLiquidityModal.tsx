...
export default function RemoveLiquidityModal({
  ...
  const [withdrawalMode, setWithdrawalMode] = useState<"percentage" | "amount">(
    "percentage"
  );
  ...
  const [token0Long, setToken0Long] = useState<WithdrawalAmount>({
    percentage: 0,
    amount: "",
    usePercentage: true,
  });
  const [token0Short, setToken0Short] = useState<WithdrawalAmount>({
    percentage: 0,
    amount: "",
    usePercentage: true,
  });
  const [token1Long, setToken1Long] = useState<WithdrawalAmount>({
    percentage: 0,
    amount: "",
    usePercentage: true,
  });
  const [token1Short, setToken1Short] = useState<WithdrawalAmount>({
    percentage: 0,
    amount: "",
    usePercentage: true,
  });
  ...
  const handleToken0ShortPercentageChange = useCallback(
    (percentage: number) => {
  ...
  const handleToken1ShortPercentageChange = useCallback(
    (percentage: number) => {
  ...
  const handleRemoveLiquidity = async () => {
    ...
    const liquidity0Long = calculateWithdrawalAmount(
      token0Long.amount,
      token0Long.percentage,
      token0Long.usePercentage,
      position.userLongX
    );
    const liquidity0Short = calculateWithdrawalAmount(
      token0Short.amount,
      token0Short.percentage,
      token0Short.usePercentage,
      position.userShortX
    );
    const liquidity1Long = calculateWithdrawalAmount(
      token1Long.amount,
      token1Long.percentage,
      token1Long.usePercentage,
      position.userLongY
    );
    const liquidity1Short = calculateWithdrawalAmount(
      token1Short.amount,
      token1Short.percentage,
      token1Short.usePercentage,
      position.userShortY
    );

    // Check if any amount is being withdrawn
    const totalWithdrawal =
      parseFloat(liquidity0Long) +
      parseFloat(liquidity0Short) +
      parseFloat(liquidity1Long) +
      parseFloat(liquidity1Short);
          ...
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="text-sm font-light text-gray-600 dark:text-gray-400">
                Withdraw your liquidity position for {position.token0Symbol}/
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div className="text-center">
                        <div className="text-sm text-gray-600 dark:text-gray-400 mb-1">
                          {transactionResult.token0Symbol} Withdrawn
                        </div>
                        <div className="font-medium text-gray-900 dark:text-white">
                          {parseFloat(transactionResult.amount0 || "0").toFixed(
                            6
                          )}
                        </div>
                      </div>
                      <div className="text-center">
                        <div className="text-sm text-gray-600 dark:text-gray-400 mb-1">
                          {transactionResult.token1Symbol} Withdrawn
                        </div>
                        <div className="font-medium text-gray-900 dark:text-white">
                          {parseFloat(transactionResult.amount1 || "0").toFixed(
                            6
                          )}
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-2 gap-2 text-xs">
                        <div className="flex justify-between">
                          <span className="text-green-600 dark:text-green-400">
                            Long {transactionResult.token0Symbol}:
                          </span>
                          <span className="font-mono">
                            {parseFloat(
                              transactionResult.liquidity0Long || "0"
                            ).toFixed(6)}
                          </span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-green-600 dark:text-green-400">
                            Long {transactionResult.token1Symbol}:
                          </span>
                          <span className="font-mono">
                            {parseFloat(
                              transactionResult.liquidity1Long || "0"
                            ).toFixed(6)}
                          </span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-red-600 dark:text-red-400">
                            Short {transactionResult.token0Symbol}:
                          </span>
                          <span className="font-mono">
                            {parseFloat(
                              transactionResult.liquidity0Short || "0"
                            ).toFixed(6)}
                          </span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-red-600 dark:text-red-400">
                            Short {transactionResult.token1Symbol}:
                          </span>
                          <span className="font-mono">
                            {parseFloat(
                              transactionResult.liquidity1Short || "0"
                            ).toFixed(6)}
                          </span>
                        </div>
                      </div>
