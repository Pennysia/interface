...
import { ROUTER_ABI, MARKET_ABI, LIQUIDITY_ABI } from "../lib/abis";
...
export default function SwapInterface({ className }: SwapInterfaceProps) {
  ...
  // Detect wrong network from Privy wallet chainId (can be EIP-155 format, hex string, or number)
  const walletChain =
    wallets && wallets.length > 0 ? (wallets[0] as any).chainId : undefined;
  const walletChainId =
    typeof walletChain === "number"
      ? walletChain
      : typeof walletChain === "string"
      ? walletChain.startsWith("eip155:")
        ? parseInt(walletChain.split(":")[1], 10) // Parse EIP-155 format (eip155:146)
        : parseInt(walletChain, 16) // Parse hex format (0x92)
      : undefined;
  ...
  console.log("🔍 Network Debug:", {
    ...
    walletChainId,
    CURRENT_CHAIN_ID,
    isSonicMainnet: walletChainId ? isSonicMainnet(walletChainId) : "undefined",
    isWrongNetwork:
      authenticated &&
      wallets.length > 0 &&
      walletChainId !== undefined &&
      !isSonicMainnet(walletChainId),
  });
  ...
  const [activeField, setActiveField] = useState<"input" | "output" | null>(
    null
  ); // Track which field user is typing in
  const [swapPrice, setSwapPrice] = useState("");
  const [slippage, setSlippage] = useState(
    PENNYSIA_CONSTANTS.DEFAULT_SLIPPAGE_BPS / 100
  ); // 0.5%
    ...
    const refreshQuote = async () => {
      if (shouldRefresh() && selectedTokenA && selectedTokenB) {
        ...
        try {
          ...
          const calculatedOutput = await calculateOutputAmount(
            inputAmount,
            selectedTokenA,
            selectedTokenB
          );
  ...
  const getPoolReserves = async (
    ...
    try {
      ...
      try {
        const [reserve0Long, reserve0Short, reserve1Long, reserve1Short] =
          await marketContract.getReserves(token0.address, token1.address);
  ...
  const calculateOutputAmount = async (
    ...
    try {
      ...
      try {
        ...
        console.warn(
          "Router contract call failed, using fallback calculation:",
          contractError
        );
        ...
        if (!reserves || reserves.reserve0 <= 0n || reserves.reserve1 <= 0n) {
          console.warn(
            "🔍 NO_LIQUIDITY detected - reserves too low:",
            reserves
          );
          return "NO_LIQUIDITY";
        }
        ...
        const reserveOut = isToken0Input
          ? reserves.reserve1
          : reserves.reserve0;
  ...
  const calculateInputAmount = async (
    ...
    try {
      ...
      try {
        // Call Router's getAmountsIn function with path (same pattern as getAmountsOut)
        const path = [tokenIn.address, tokenOut.address];

        // Debug logging for Router call
        console.log("🔍 ROUTER CONTRACT CALL DEBUG (getAmountsIn):");
        console.log("tokenIn:", tokenIn.symbol, tokenIn.address);
        console.log("tokenOut:", tokenOut.symbol, tokenOut.address);
        console.log("path:", path);
        ...
        const amountIn = amounts[0]; // Input amount is the first element

        console.log(
          "Router amounts result (getAmountsIn):",
          amounts.map((a: any) => a.toString())
        );
        console.log("Router amountIn:", amountIn.toString());
        ...
        console.log("🔍 DETAILED DEBUG - calculateInputAmount fallback:", {
        ...
        if (!reserves || reserves.reserve0 <= 0n || reserves.reserve1 <= 0n) {
          console.warn(
            "🔍 NO_LIQUIDITY detected - reserves too low (calculateInputAmount):",
            {
              reserves,
              reserve0: reserves?.reserve0?.toString(),
              reserve1: reserves?.reserve1?.toString(),
        ...
        console.log("🔍 RESERVE MAPPING DEBUG:", {
          token0: token0.symbol,
          token1: token1.symbol,
          isToken0Input,
        ...
        // Check if requested output exceeds reasonable limit (99% of reserves to maintain pool stability)
        const maxOutputAmount = (reserveOut * 99n) / 100n; // 99% of reserves
        if (amountOut >= maxOutputAmount) {
          console.warn(
            "🔍 Requested output amount exceeds reasonable limit (99% of reserves)",
            {
              amountOut: amountOut.toString(),
              reserveOut: reserveOut.toString(),
              maxOutputAmount: maxOutputAmount.toString(),
              percentageOfReserves: Number((amountOut * 100n) / reserveOut),
              tokenOut: tokenOut.symbol,
              outputAmountValue,
            }
          );
  ...
  const handleInputAmountChange = async (value: string) => {
    ...
    console.log("📝 User input amount change:", {
      value,
    ...
    try {
      ...
      if (!isUpdating.current.output) {
        // Only update if output isn't being actively edited
  ...
  const handleOutputAmountChange = (value: string) => {
    ...
    console.log("📝 User output amount change:", {
      value,
      source: "direct_user_input",
    });
    ...
    console.log(
      "🔄 User typing in output field - calculating required input amount"
    );
    ...
    calculationTimeoutRef.current = setTimeout(async () => {
      try {
        ...
        const calculatedInput = await calculateInputAmount(
          value,
          selectedTokenA,
          selectedTokenB
        );
        ...
        if (!isUpdating.current.input) {
          console.log(
            "🔄 Auto-updating input amount from output calculation:",
            {
  ...
  useEffect(() => {
    ...
    const calculateOutput = async () => {
      try {
        ...
        try {
          const inputAmountWei = parseTokenAmount(
            inputAmount,
            selectedTokenA.decimals
          );
          const mockOutputWei = (
            (BigInt(inputAmountWei) * BigInt(95)) /
            BigInt(100)
          ).toString();
          const formattedOutput = formatTokenAmount(
            mockOutputWei,
    ...
    // Set new timeout
    timeoutRef.current = setTimeout(calculateOutput, 150); // Debounce 150ms
  ...
  const handleSwap = async () => {
    ...
    try {
      ...
      if (!marketAddress || !routerAddress) {
        toast.error(
          "Market or Router address not found for the current chain."
        );
        setIsSwapping(false);
        return;
      }
      ...
      const safeParseUnits = (value: string, decimals: number): bigint => {
        try {
          ...
          if (parts.length > 1 && parts[1].length > 18) {
            ...
            safeValue =
              parts[0] + "." + parts[1].substring(0, 18).replace(/0+$/, "");
            console.log(
              `⚠️ Truncated swap value from ${value} to ${safeValue} to prevent ethers.js underflow`
            );
          }
      ...
      const inputAmountWei = safeParseUnits(
        inputAmount,
        selectedTokenA.decimals
      );
      let standardParseWei = 0n;
      try {
        standardParseWei = ethers.parseUnits(
          inputAmount,
          selectedTokenA.decimals
        );
      } catch (standardError) {
      ...
      console.log("🔍 Amount parsing comparison:", {
        ...
        safeBackToString: ethers.formatUnits(
          inputAmountWei,
          selectedTokenA.decimals
        ),
        standardBackToString:
          standardParseWei > 0n
            ? ethers.formatUnits(standardParseWei, selectedTokenA.decimals)
            : "failed",
        resultsMatch: inputAmountWei === standardParseWei,
        recommendedUse:
          inputAmountWei === standardParseWei
            ? "Either"
            : "Check which is correct",
      });

      // Use standard parsing if both match, otherwise use safe parsing
      const finalInputAmountWei =
        inputAmountWei === standardParseWei && standardParseWei > 0n
          ? standardParseWei
          : inputAmountWei;
      ...
      const expectedOutputWei = safeParseUnits(
        outputAmount,
        selectedTokenB.decimals
      );
      ...
      const minOutputAmount =
        (expectedOutputWei * BigInt(10000 - slippageBps)) / BigInt(10000);
      ...
      console.log("💱 Swap calculation details:", {
        ...
        minOutputAmountFormatted: ethers.formatUnits(
          minOutputAmount,
          selectedTokenB.decimals
        ),
      ...
      const deadline =
        ...
        PENNYSIA_CONSTANTS.DEFAULT_DEADLINE_SECONDS;
      ...
      try {
        ...
        console.log("🔍 Actual market simulation result:", {
          ...
          actualMarketOutputFormatted: ethers.formatUnits(
            actualMarketOutput,
            selectedTokenB.decimals
          ),
          ...
          difference:
            (
              ((BigInt(actualMarketOutput) - BigInt(expectedOutputWei)) *
                100n) /
              BigInt(expectedOutputWei)
            ).toString() + "%",
        });

        // Use the actual market output with user's slippage setting
        const slippageMultiplier = BigInt(
          Math.floor((100 - effectiveSlippage) * 100)
        ); // Convert to basis points
        const actualMinOutput =
          (BigInt(actualMarketOutput) * slippageMultiplier) / 10000n;
        console.log("📊 Using actual market output for slippage:", {
          ...
          finalMinOutputFormatted: ethers.formatUnits(
            actualMinOutput,
            selectedTokenB.decimals
          ),
        });
        ...
        console.log("🚀 Executing swap after simulation with parameters:", {
          ...
          mustMatch:
            "nativeTokenValue must equal finalInputAmountWei for native swaps",
        ...
        if (selectedTokenA.address === ethers.ZeroAddress) {
          console.log("🔍 CRITICAL: Native token swap validation:", {
            contractFunction:
              "swap(uint256 amountIn, uint256 amountOutMinimum, address[] path, address to, uint256 deadline)",
            parameterAmountIn: finalInputAmountWei.toString(),
            transactionMsgValue: nativeTokenValue.toString(),
            MUST_BE_EQUAL:
              "These two values must be exactly equal for native swaps",
            equalityCheck: finalInputAmountWei === nativeTokenValue,
            difference:
              finalInputAmountWei > nativeTokenValue
                ? `amountIn is ${
                    finalInputAmountWei - nativeTokenValue
                  } wei larger`
        ...
        try {
          ...
          if (contractError?.data) {
            ...
            if (contractError.data === "0x3e032a3b") {
              ...
              errorMessage =
                "Forbidden - insufficient permissions or invalid operation";
              ...
              errorMessage =
                "Custom contract error (0xc24d2823) - possible insufficient liquidity or invalid parameters";
            }
          }

          throw new Error(
            `Swap failed: ${errorMessage}. Original error: ${
      ...
      if (receipt.status === 1) {
        ...
        try {
          ...
          console.warn(
            "Could not extract actual swap amount from transaction:",
            error
          );
        }
  ...
  if (!ready) {
    return (
      <div
        className={clsx(
          "relative bg-gray-800/50 backdrop-blur-sm rounded-2xl border border-gray-800 p-4 w-full max-w-md mx-auto",
          className
        )}
      >
  ...
  return (
    <>
      <div
        className={clsx(
          "relative dark:bg-[var(--background)] backdrop-blur-sm rounded-2xl border border-gray-800 p-4 w-full max-w-md mx-auto",
          className
        )}
      >
        ...
        <div className="space-y-2 mb-2">
          <div className="relative bg-gray-200/50 dark:bg-gray-800/30 rounded-xl p-4">
            <div className="flex items-center justify-between mb-2">
              ...
              <div className="flex items-center space-x-2">
                <span className="text-sm text-gray-400">
                  Balance:{" "}
                  {selectedTokenA ? selectedTokenA.balance || "0.00" : "0.00"}
                </span>
                {selectedTokenA?.balance &&
                  parseFloat(selectedTokenA.balance) > 0 && (
                    <button
                      onClick={() => {
                      ...
                      disabled={
                        isSwapping || isCalculatingInput || isCalculatingOutput
                      }
                    >
                      Max
                    </button>
            ...
            <div className="flex items-center gap-4">
              <input
                ...
                inputMode="decimal"
                value={
                  isCalculatingInput && activeField !== "input"
                    ? "Calculating..."
                    : inputAmount
                }
                onChange={(e) => {
          ...
          <div className="relative bg-gray-200/50 dark:bg-gray-800/30 rounded-xl p-4">
            <div className="flex items-center justify-between mb-2">
              ...
              <span className="text-sm text-gray-400">
                Balance:{" "}
                {selectedTokenB ? selectedTokenB.balance || "0.00" : "0.00"}
              </span>
            </div>
            <div className="flex items-center gap-4">
              <input
                ...
                inputMode="decimal"
                value={
                  isCalculatingOutput && activeField !== "output"
                    ? "Calculating..."
                    : outputAmount === "NO_LIQUIDITY"
        ...
        {swapPrice && (
          <div className=" p-3 rounded-lg space-y-1">
            <div className="flex items-center justify-between text-sm">
              <span className="text-gray-600 dark:text-gray-400 font-light text-xs">
                Average Price:
              </span>
              <span className="text-gray-900 dark:text-gray-200 font-light text-xs">
                1 {selectedTokenA?.symbol} = {swapPrice}{" "}
                {selectedTokenB?.symbol}
              </span>
            </div>
            <div className="flex items-center justify-between text-sm">
              ...
              <span className="text-gray-900 dark:text-gray-200 font-light text-xs">
                ...
                {(1 / parseFloat(swapPrice) || 0).toFixed(6)}{" "}
                {selectedTokenA?.symbol}
              </span>
            </div>
          </div>
            ...
            <div className="bg-[var(--background)] dark:bg-yellow-500/10 border border-yellow-500/20 rounded-lg p-4 flex items-center space-x-3 my-4">
              ...
              <div className="flex-1">
                ...
                <p className="text-yellow-300/80 text-xs mt-1">
                  There is no liquidity pool for {selectedTokenA.symbol}/
                  {selectedTokenB.symbol}. Consider adding liquidity first or
                    ...
                    <div className="bg-gray-700/50 rounded-lg p-4 space-y-3">
                      <div className="flex justify-between">
                        ...
                        <span className="text-white font-medium">
                          {transactionResult.amountIn}{" "}
                          {transactionResult.tokenInSymbol}
                        </span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-gray-400">You Received:</span>
                        <span className="text-white font-medium">
                          {transactionResult.amountOut}{" "}
                          {transactionResult.tokenOutSymbol}
                        </span>
                      </div>
